...
contract MMPR{
    ...
    uint[3] public representation;  // representation of parties in parliment
    ...
    function result() public {
        ...
        for(uint i=0; i<parties; i++){
            while(representation[i]*totalVotes <= secondVotes[i]*size  && occupied <= size){
                representation[i] += 1;
                occupied += 1;
            }
        }
